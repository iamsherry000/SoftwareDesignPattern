...
​
  時，依序執行以下步驟：
(P) 印出狀態列： 首先印出此角色的狀態列（詳見「輸出格式」章節）。注意：此時印出的各項屬性值為結算本回合狀態效果前的數值。
(E) 結算狀態效果： 依據角色當前狀態結算效果——若為石化，此角色本回合無法行動，直接略過 S1、S2、S3；若為中毒，角色的 HP 減少 30，若減少後 HP ≤ 0，則此角色死亡且無法行動，略過 S1、S2、S3。正常狀態和受到鼓舞狀態在此步驟中不需結算效果。
...
=
0
n=0 的情況），則 RPG 會自動決定目標為該留存的 
n
n 位候選角色並且略過 S2 （但仍會執行 S3，MP 照常扣除；若 
n
=
0
n=0 則技能不會對任何角色產生效果）。
...
石化 (Petrochemical)：在步驟 (E) 結算狀態效果時，此角色本回合無法行動，S1、S2 和 S3 將會被略過。
中毒 (Poisoned)：在步驟 (E) 結算狀態效果時，此角色的 HP 會減少 30，如果 HP 減少後 ≤ 0，則此角色死亡且無法行動（略過 S1、S2、S3）。注意：步驟 (P) 印出的 HP 為扣除中毒傷害前的值。
...
自爆 (SelfExplosion)： (MP: 200 — 目標：戰場上所有仍存活的角色) - 行動角色自殺（行動角色生命值歸零），接著對戰場上所有仍存活的角色造成 150 點傷害。
鼓舞 (Cheerup)： (MP: 100 — 目標：3 位友軍＊（不含行動角色自身）) - 使所有目標角色獲得三回合的受到鼓舞狀態（包含當前回合）
...
如果目標角色的當前狀態為中毒狀態或是石化狀態，對目標角色造成三次 80 點傷害。（每一次傷害後會判定目標是否死亡，若已死亡則不再造成剩餘次數的傷害。）
如果目標角色的當前狀態為受到鼓舞狀態，對目標角色造成 100 點傷害，並將目標角色的狀態恢復成正常狀態。
如果目標角色的當前狀態為正常狀態，對目標角色造成 100 點傷害。
有多項技能都能「改變角色三回合內的狀態」，由於每個角色最多只能處於一種狀態之下，每當角色獲得新的狀態時，就會覆寫舊有狀態，並且重新倒數三回合（含當前回合）。三回合過後（含當前回合），角色的狀態會還原到正常狀態。舉例：若在第 5 回合施放，效果持續第 5、6、7 回合（共 3 個回合），第 8 回合開始恢復正常狀態。
...
m
m 位），選擇完所有目標角色後 seed 的值才會加一。舉例來說：如果 seed=2、候選角色有三名：(A, B, C)，且 AI 要選擇 2 名目標角色，則他會選擇 (C, A)，選完之後 seed=3。
...
<角色名稱>, <HP>, <MP>, <STR> 和 <狀態> 為角色的名稱、HP、MP、STR 和狀態。其中 <HP> 為結算本回合狀態效果前的生命值（例如中毒角色印出的 HP 尚未扣除本回合的中毒傷害）。<狀態> 的值只可能為 正常, 石化, 中毒, 受到鼓舞 中其中一項。
...
在進入階段 S3 時，如果此行動為普通攻擊，則輸出一行訊息： <角色> 攻擊 <目標>。 ；如果此行動為自我治療，則輸出一行訊息：<角色> 使用了 <技能>。 ；如果此行動的目標角色數為 0（無任何目標），則輸出一行訊息：<角色> 使用了 <技能>。 ；除此之外則輸出一行訊息：<角色> 對 <目標角色清單> 使用了 <技能>。
